        ...
        html += `
                    ...
                    <tfoot class="table-secondary">
                        <tr>
                            ...
                            <td class="fw-bold" id="total-orders">${totalOrders}</td>
...
function removeService(service_id) {
    ...
    // Obtener información de configuraciones y órdenes
    let configsCount = 0;
    let ordersCount = 0;
    let ordersByStatus = {};
    ...
    let message = `¿Está seguro de que desea eliminar el servicio "${serviceName}"?\n\n`;

    if (configsCount > 0) {
        message += `📋 Configuraciones: ${configsCount}\n`;
    }

    if (ordersCount > 0) {
        message += `📦 Órdenes de servicio: ${ordersCount}\n`;

        // Agregar desglose de órdenes por estado si existen
        if (Object.keys(ordersByStatus).length > 0) {
            message += `\nDesglose de órdenes:\n`;
            if (ordersByStatus[1])
                message += `• Pendientes: ${ordersByStatus[1]}\n`;
            if (ordersByStatus[2])
                message += `• En progreso: ${ordersByStatus[2]}\n`;
            if (ordersByStatus[3])
                message += `• Completadas: ${ordersByStatus[3]}\n`;
            if (ordersByStatus[4])
                message += `• Canceladas: ${ordersByStatus[4]}\n`;
        }
    }

    message += `\n⚠️  ADVERTENCIA: Esta acción eliminará todas las configuraciones y órdenes asociadas a este servicio.`;
    ...
    // Manejar configuración de servicios
    if (typeof services_configuration !== "undefined") {
    ...
    // Mostrar mensaje de éxito
    let successMessage = `✅ Servicio "${serviceName}" eliminado correctamente.`;
    if (configsCount > 0) {
        successMessage += `\n• ${configsCount} configuración(es) eliminada(s)`;
    }
    if (ordersCount > 0) {
        successMessage += `\n• ${ordersCount} orden(es) de servicio eliminada(s)`;
    }
...
// Función para obtener el resumen de órdenes por servicio 
...
// Función para actualizar el resumen de un servicio específico
function updateServiceSummary(service_id) {
    ...
    // Opcional: Actualizar tooltip o información adicional sobre estados
...
// Función para obtener el resumen general de todos los servicios
