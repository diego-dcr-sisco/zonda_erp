                ...
                <div class="row">
                    <div class="col-lg-6 col-12 mb-3">
                        ...
                        <input type="text" class="form-control " id="name" name="name" placeholder="Example"
                            maxlength="50" value="{{ $customer->name }}" required>
                    </div>
                    <div class="col-lg-6 col-12 mb-3">
                        <label for="email" class="form-label">{{ __('customer.data.email') }}: </label>
                        <input type="email" class="form-control" id="email" name="email" maxlength="50" value="{{ $customer->email }}"
                    ...
                    <div class="col-lg-6 col-12 mb-3">
                        ...
                        <input type="text" class="form-control" id="address" name="address" maxlength="50" value="{{ $customer->address }}"
                            placeholder="#00 Col. Example" required>
                    </div>
                    <div class="col-lg-3 col-12 mb-3">
                        <label for="phone" class="form-label is-required">{{ __('customer.data.phone') }}</label>
                        <input type="text" min=1 class="form-control" id="phone" placeholder="0000000000" value="{{ $customer->phone }}"
                    ...
                    <div class="col-lg-3 col-12 mb-3">
                        ...
                        <input type="text" class="form-control" name="zip_code" placeholder="00000" value="{{ $customer->zip_code }}"
                    ...
                    <div class="col-lg-3 col-12 mb-3">
                        ...
                        <select class="form-select " id="state" name="state" onchange="load_city()" required>
                            ...
                            @foreach ($states as $state)
                                <option value="{{ $state['key'] }}" {{ $customer->state == $state['key'] ? 'selected' : '' }}>{{ $state['name'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-lg-3 col-12 mb-3">
                        ...
                        <select type="text" class="form-select " id="service-type" name="service_type_id" required>
                            @foreach ($service_types as $service)
                                <option value="{{ $service->id }}"
                                    {{ $customer->service_type_id == $service->id ? 'selected' : '' }}>
                                    {{ $service->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select type="text" class="form-select" name="company_category_id">
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                    {{ $customer->company_category_id == $category->id ? 'selected' : '' }}>
                    ...
                    <div class="col-lg-3 col-12 mb-3">
                        ...
                        <select class="form-select" name="contact_medium" id="contact-medium">
                            @foreach ($contact_medium as $key => $medium)
                                <option value="{{ $key }}" {{ $key == $customer->contact_medium ? 'selected' : '' }}>{{ $medium }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-12 mb-3">
                        ...
                        <input type="text" class="form-control" id="map_location_url" name="map_location_url" value="{{ $customer->map_location_url }}"
                            placeholder="https://www.google.com/maps?q=latitude,longitude&hl=en" />
                    </div>
                </div>
            </div>

            <input type="hidden" name="general_sedes" value="{{ $customer->general_sedes }}">
