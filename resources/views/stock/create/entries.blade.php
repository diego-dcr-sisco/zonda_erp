...
<div class="container-fluid p-5">
    <div class="container">
        <form class="content" action="{{ route('stock.storeEntry') }}" method="POST">
            @csrf
            @method('PUT')
            <div class="body">
                <div class="mb-3">
                    ...
                    <div class="col-4">
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="lot" name="lot_id" required>
                                {{-- aqui se muestran las opciones segun el producto seleccionado  --}}
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-3 d-flex justify-content-between">
                <button type="submit" class="btn btn-primary px-5" style="width: 90%">Registrar Entrada</button>
                <a href="{{ url()->previous() }}" class="btn btn-outline-danger" onclick="return confirm('¿Está seguro que desea cancelar?')">{{ __('buttons.cancel') }}</a>
            </div>
        </form>
    </div>
</div>
