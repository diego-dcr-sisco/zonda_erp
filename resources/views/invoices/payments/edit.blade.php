...
@section('content')
    <div class="row m-0">
        ...
        <form class="form p-3" action="{{ route('invoices.payments.update', $paymentComplement->id) }}" method="POST" enctype="">
            ...
            <!-- Inputs hidden para almacenar los IDs existentes -->
            <input type="hidden" id="existing_payment_item_ids" value="{{ json_encode($paymentsData['Complemento']['Payments'] ?? []) }}">
                    ...
                    <div class="border rounded shadow p-3 mb-3">
                        <div class="row">
                            ...
                            <div class="col-lg-6 mb-3">
                                <label class="form-label is-required" for="expedition_place">Lugar de expedición (Código postal)</label>
                                <input type="text" class="form-control" id="expedition_place" name="expedition_place"
                                    value="{{ $paymentComplement->expedition_place ?? $sat_config['zip_code'] }}" required>
                                <div class="form-text" id="expedition_place_help">Código postal de expedición</div>
                            </div>
                            <div class="col-lg-6 mb-3">
                                <label class="form-label is-required" for="business_name">Razon social</label>
                                <input type="text" class="form-control" id="business_name" name="business_name"
                                    value="{{ $sat_config['business_name'] }}" readonly>
                                <div class="form-text">Razón social del emisor</div>
                            </div>
                            <div class="col-12 mb-3">
                                <label class="form-label is-required" for="rfc">Registro Federal Contribuyente (RFC)</label>
                                <input type="text" class="form-control" id="rfc" name="rfc"
                                    value="{{ $sat_config['rfc'] }}" readonly>
                                <div class="form-text">RFC del emisor</div>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <input type="text" class="form-control" id="receiver_name" name="receiver_name"
                                    value="{{ $paymentComplement->receiver_name ?? '' }}" required>
                            </div>
                            <div class="col-lg-6 col-12 mb-3">
                                ...
                                <input type="text" class="form-control" id="receiver_rfc" name="receiver_rfc"
                                    value="{{ $paymentComplement->receiver_rfc ?? '' }}" required>
                            </div>
                            <div class="col-lg-6 col-12 mb-3">
                                ...
                                <input type="text" class="form-control" id="receiver_tax_zip_code"
                                    name="receiver_tax_zip_code" value="{{ $paymentComplement->receiver_tax_zip_code ?? '' }}" required>
                            </div>
                            <div class="col-lg-6 col-12 mb-3">
                                ...
                                <select name="receiver_cfdi_use" id="receiver_cfdi_use" class="form-select" required>
                                    ...
                                    @foreach ($cfdiUsages as $usage)
                                        <option value="{{ $usage['Value'] }}" 
                                            {{ ($paymentComplement->receiver_cfdi_use ?? '') == $usage['Value'] ? 'selected' : '' }}>
                            ...
                            <div class="col-lg-6 col-12 mb-3">
                                ...
                                <select name="receiver_fiscal_regime" id="receiver_fiscal_regime" class="form-select" required>
                                    <option value="" disabled>Seleccione el regimen</option>
                                    @foreach ($taxRegimes as $taxRegime)
                                        <option value="{{ $taxRegime['Value'] }}"
                                            {{ ($paymentComplement->receiver_fiscal_regime ?? '') == $taxRegime['Value'] ? 'selected' : '' }}>
            ...
            <a href="{{ route('invoices.payments.index') }}" class="btn btn-secondary">
                Cancelar
            </a>
        </form>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Definir taxObjects
            const taxObjects = @json($taxObjects);

            // Obtener IDs existentes
            const existingPaymentData = JSON.parse($('#existing_payment_item_ids').val() || '[]');

            // Inicializar con datos existentes si están disponibles
            @if(isset($paymentsData) && isset($paymentsData['Complemento']['Payments']))
                loadExistingPayments(@json($paymentsData));
            @else
            ...
            function loadExistingPayments(data) {
                if (data.Complemento && data.Complemento.Payments) {
                    data.Complemento.Payments.forEach((paymentData, paymentIndex) => {
                        const paymentId = 'payment_' + Date.now() + '_' + paymentIndex;
                        
                        // Crear el HTML del pago
                        const paymentHtml = `
                            <div class="payment-item border rounded p-3 mb-3" data-payment-id="${paymentId}" data-payment-item-id="${paymentData.PaymentItemId || ''}">
                                ...
                                <div class="row">
                                    <div class="col-md-3 mb-3">
                                        ...
                                        <input type="datetime-local" class="form-control payment-date" 
                                               value="${paymentData.Date.replace('Z', '')}" required>
                                    </div>
                                    <div class="col-md-3 mb-3">
                                        ...
                                        <select class="form-select payment-form" required>
                                            ...
                                            @forelse ($paymentForms as $index => $form)
                                                <option value="{{ $index }}" ${paymentData.PaymentForm == '{{ $index }}' ? 'selected' : ''}>{{ $index }} - {{ $form }}</option>
                                            @empty
                                    ...
                                    <div class="col-md-3 mb-3">
                                        ...
                                        <select class="form-select payment-method" required>
                                            ...
                                            @forelse ($paymentMethods as $index => $method)
                                                <option value="{{ $index }}" ${paymentData.RelatedDocuments[0]?.PaymentMethod == '{{ $index }}' ? 'selected' : ''}>{{ $index }} - {{ $method }}</option>
                                            @empty
                                    ...
                                    <div class="col-md-3 mb-3">
                                        ...
                                        <select class="form-select payment-currency" required>
                                            <option value="MXN" ${paymentData.Currency == 'MXN' ? 'selected' : ''}>MXN - Peso Mexicano</option>
                                            <option value="USD" ${paymentData.Currency == 'USD' ? 'selected' : ''}>USD - Dólar Americano</option>
                                            <option value="EUR" ${paymentData.Currency == 'EUR' ? 'selected' : ''}>EUR - Euro</option>
                                        </select>
                                    </div>
                                </div>
                        ...
                        const paymentInvoices = paymentData.RelatedDocuments.map(doc => {
                            const tax = doc.Taxes && doc.Taxes.length > 0 ? doc.Taxes[0] : {
                                Rate: 0.16,
                                Total: 0,
                                Base: 0
                            };
                            ...
                            return {
                                id: doc.InvoiceId,
                                folio: `${doc.Serie}-${doc.Folio}`,
                                uuid: doc.Uuid,
                                receiver: data.Receiver.Name,
                                rfc: data.Receiver.Rfc,
                                expedition_place: data.ExpeditionPlace,
                                receiver_zip: data.Receiver.TaxZipCode,
                                cfdi_usage: data.Receiver.CfdiUse,
                                fiscal_regime: data.Receiver.FiscalRegime,
                                total: doc.PreviousBalanceAmount,
                                serie: doc.Serie,
                                folio_num: doc.Folio,
                                tax_rate: tax.Rate || 0.16,
                                tax_total: tax.Total || 0,
                                subtotal: tax.Base || (doc.PreviousBalanceAmount - (tax.Total || 0)),
                                tax_object: doc.TaxObject || '02',
                                // Datos específicos del pago
                                partiality_number: doc.PartialityNumber,
                                previous_balance: doc.PreviousBalanceAmount,
                                amount_paid: doc.AmountPaid,
                                outstanding_balance: doc.ImpSaldoInsoluto,
                                // IDs existentes
                                related_doc_id: doc.RelatedDocId || null
                            };
                        });

                        payments.push({
                            id: paymentId,
                            payment_item_id: paymentData.PaymentItemId || null,
                            invoices: paymentInvoices
                        });

                        // Actualizar la visualización de facturas
            ...
            function addNewPayment() {
                ...
                payments.push({
                    id: paymentId,
                    payment_item_id: null, // Nuevo pago no tiene ID existente
                    invoices: []
                });
            ...
            function createInvoicePaymentConfig(invoice, paymentId, index) {
                ...
                const partialityNumber = invoice.partiality_number || (index + 1);
                const previousBalance = invoice.previous_balance || total;
                const amountPaid = invoice.amount_paid || total;
                const outstandingBalance = invoice.outstanding_balance || 0;
                const relatedDocId = invoice.related_doc_id || null;
                ...
                return `
                    <div class="invoice-payment-item border rounded p-2 mb-2" data-invoice-id="${invoice.id}" data-related-doc-id="${relatedDocId || ''}">
                        ...
                        <div class="row">
                            ...
                            <div class="col-6 mb-1">
                                ...
                                <input type="number" class="form-control form-control-sm outstanding-balance" 
                                       value="${parseFloat(outstandingBalance).toFixed(2)}" step="0.01" min="0" required>
                            </div>
                        </div>
                ...
                $('.modal-invoice-checkbox:checked').each(function() {
                    if (verifyRfcConsistency(currentPayment.invoices, $(this).data('rfc'))) {
                        const invoice = {
                            ...
                            related_doc_id: null // Nuevas facturas no tienen RelatedDocId
                        };
            ...
            function buildPaymentsJson() {
                ...
                const paymentsArray = payments.map(payment => {
                    ...
                    const relatedDocuments = payment.invoices.map((invoice, index) => {
                        ...
                        const relatedDocId = item.data('related-doc-id') || null;
                        ...
                        return {
                            "RelatedDocId": relatedDocId, // Incluir RelatedDocId
                    ...
                    return {
                        "PaymentItemId": payment.payment_item_id, // Incluir PaymentItemId
